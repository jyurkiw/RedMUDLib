...
function RoomLib(client) {
    ...
    //addConnection(exitDir, westernOverlook, goblinCaveEntrance, function()
    function setConnection(exitCommand, sourceRoom, destinationRoom, callback) {
        var sourceCode = getRoomCode(sourceRoom);
        var destinationCode = getRoomCode(destinationRoom);

        client.hmset(codeutil.convertRoomToExitsCode(sourceCode), exitCommand, destinationCode, function(err, res) {
            if (typeof(callback) === 'function') {
                callback();
            }
        });
